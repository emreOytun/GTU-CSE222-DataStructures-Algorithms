...
public class Account {
    ...
    public void login(List<Account> accounts) {        
        if (isLoginned) {
            ...
            for (int i = 0; i < accounts.size() && (!someoneLogin || !signedUp); ++i) {
   ...
   /**
    ...
    public void blockAccount(Account account) {
        if (!isLoginned) {
            ...
            for (int i = 0; i < posts.size(); ++i) {
                posts.get(i).removeInteractions(account.getAccountId());
            }
     ...
     * @param accounts The registered account's array to check if there is such post and find the post's owner.
    ...
    private boolean isBlocked(String username) {
        ...
        boolean res = false;
        for (int i = 0; i < blockedAccounts.size() && !res; ++i) {
            if (blockedAccounts.get(i).getUsername().equals(username)) {
                res = true;
            }
        }
        return res;
    ...
    private void viewInteractions(List<Account> accounts) {
        for (int i = 0; i < posts.size(); ++i) {
            posts.get(i).viewInteractions(accounts);
        }
    }
    ...
    private Post findPostById(List<Account> accounts, int postId) {
        ...
        for (int i = 0; i < accounts.size() && !isFound; ++i) {
            ...
            for (int j = 0; j < account.posts.size() && !isFound; ++j) {
                Post element = account.posts.get(j);
    ...
    private int getLastInteractionId(List<Account> accounts) {
        ...
        for (int i = 0; i < accounts.size(); ++i) {
            Account a = accounts.get(i);
            for (int j = 0; j < a.posts.size(); ++j) {
                Post p = a.posts.get(j);
